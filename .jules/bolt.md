...
## 2025-06-25 - Avoid optimizing calculate_zeros with simple formulas
**Learning:** In python-control, computing zeros for a StateSpace system using $A - B D^{-1} C$ mathematically computes *invariant zeros* (which incorrectly includes unobservable and uncontrollable modes for non-minimal systems), instead of *transmission zeros*. Moreover, it's numerically unstable for ill-conditioned $D$ matrices. Hardcoding `dtype=float` when constructing matrix pencils discards imaginary parts, causing functional regressions for complex matrices. The performance gain of avoiding `np.block` is negligible since the eigenvalue computation is $O(N^3)$, whereas the block construction is only $O(N^2)$. Furthermore, doing so prevents delegation to the highly robust `slycot` Fortran library (`ab08nd`) when available.
**Action:** Do not attempt to bypass `ct.zeros` for StateSpace systems by manually computing eigenvalues of $A - B D^{-1} C$ or manually building matrix pencils. This creates functional and numerical regressions without meaningful performance improvement.
